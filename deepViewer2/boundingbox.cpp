...
QColor nerve_color(130, 224, 170);
QColor lowercase_color(210, 180, 222);
QColor jawbone_color(252, 202, 40);
QColor mentalnerve_color(79, 195, 247);
QColor end_lowercase_color(179, 105, 47);
QColor background_color(98, 0, 234);
...
bool QBoxitem::CheckBalanceBox(QList<QBoxitem> boxes, ImgStatus &imgstatus, int &nerveCounter, int &lowercaseCounter)
{
    ...
    foreach (QBoxitem box, boxes) {
        ...
        else if( box.getClass() == LOWERCASE )
        {
...
void QBoxitem::init_map_box()
{
    ...
    map_boxclas[JAWBONE] = "jawbone";
    map_boxclas[MENTALNERVE] = "mentalnerve";
    map_boxclas[END_LOWERCASE] = "endlowercase";



//    { BACKGROUND, NERVE, LOWERCASE, JAWBONE, MENTALNERVE};
}
...

{
    ...
    if( id_class == QBoxitem::NERVE )
        mypen = QPen(nerve_color, 2);
    else if( id_class == QBoxitem::LOWERCASE )
        mypen = QPen(lowercase_color,2);
    else if( id_class == QBoxitem::JAWBONE )
        mypen = QPen(jawbone_color, 2);
    else if( id_class == QBoxitem::MENTALNERVE )
        mypen = QPen(mentalnerve_color, 2);
    else if( id_class == QBoxitem::END_LOWERCASE)
        mypen = QPen(end_lowercase_color, 2);
    else
        mypen = QPen(background_color, 2);
    this->setPen(mypen);
}
