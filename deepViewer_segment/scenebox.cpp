...
void SceneItems::_DeleteAllBoxItems()
{
    foreach(QGraphicsItem* item, this->items()){
        ...
        PolygonContour *polycontour = qgraphicsitem_cast<PolygonContour*>(item);
        if( rect != nullptr )
        {
            removeItem(item);
            delete item;
        }
        if( polycontour != nullptr )
        {
            removeItem(polycontour);
            delete polycontour;
        }
    }
}
...
//    QPolygonF contour;
//    contour.push_back(QPoint(100,100));
//    contour.push_back(QPoint(300,100));
//    contour.push_back(QPoint(100,300));
//    PolygonContour *testcontour = new PolygonContour(nullptr);

//    testcontour->setPolygon(contour);
//    this->addItem(testcontour);
...
void SceneItems::makeItemsControllable(bool areControllable){
    foreach(QGraphicsItem* item, items()){
        ...
        PolygonContour *polygonitem =  qgraphicsitem_cast<PolygonContour*>(item);
        if( imgitem != nullptr )
        {
        ...
        {
            polygonitem->setFlag(QGraphicsItem::ItemIsSelectable, areControllable);
...
void SceneItems::mousePressEvent(QGraphicsSceneMouseEvent *event){
    ...
    if(sceneMode == DrawLine)
    {
        ...
        qDebug()<<__FUNCTION__<<time.second()<<time.msec()<<time.msecsSinceStartOfDay();
...
//        qDebug()<<origPoint<<pixmapitem->sceneBoundingRect();
        ...
        if( !polyconcontour && _isInsideImage(origPoint))
        {
            qDebug()<<__FUNCTION__<<"polygon created";
            polyconcontour = new PolygonContour(nullptr);
            polyconcontour->setPos(0,0);
            polyconcontour->PutPoints(event->scenePos());



            this->addItem(polyconcontour);



        }
        else if( polyconcontour && _isInsideImage(origPoint))
        {
...
void SceneItems::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event)
{
    if( polyconcontour )
    {
        polyconcontour->closePath();

        QRectF imgRect = pixmapitem->sceneBoundingRect();
        QSize origin_size = pixmapitem->m_imgSize;

        QPolygonF restrocontour = polyconcontour->RestoreOriginalDomain(&polyconcontour->GetPolygon(), imgRect, origin_size);
        emit signalAddContour(&restrocontour);
    }
    polyconcontour = 0;
}
...
void SceneItems::keyPressEvent(QKeyEvent *event){
    ...
    if(event->key() == Qt::Key_Delete)
        foreach(QGraphicsItem* item, selectedItems()){
            BoundingBox *rect = qgraphicsitem_cast<BoundingBox*>(item);
            PolygonContour *cntrItem = qgraphicsitem_cast<PolygonContour*>(item);
            if( rect != nullptr )
            {                
//                QUuid id = ();
...
bool SceneItems::eventFilter(QObject * obj, QEvent * event)
{
//    qDebug()<<__FUNCTION__;
    if(event->type()==QEvent::KeyPress) {
        ...
        if( pressedKeys.contains(Qt::Key_Control) && pressedKeys.contains(Qt::Key_Z) )
        {
            qDebug()<<__FUNCTION__<<"key combo";
            // D and W are pressed
            if( polyconcontour )
            {
                polyconcontour->PopPoints();
...
//            else if( _previousContour )
//            {
//                _previousContour->PopPoints();
//            }
            return true;
        }
