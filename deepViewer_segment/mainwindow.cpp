...
//    fileListWidget->mouse
    ...
    connect(scene,SIGNAL(signalAddContour(QPolygonF*)),this,SLOT(addContourToViwer(QPolygonF*)));
...
void MainWindow::fileListClicked(QListWidgetItem *items)
{
    if( m_isFileListDeleting == false )
    {
        ...
        qDebug()<<__FUNCTION__<<this->_GetStatusImg();
        JsonBoxSaver::saveJsonContours(contoursList.at(this->_GetStatusImg()),"test");
...
//        int diff_ind = qAbs(file_num-this->_GetStatusImg());
        qDebug()<<__FUNCTION__<<this->_GetStatusImg()<< file_num;
        JsonBoxSaver::saveJsonContours(contoursList.at(this->_GetStatusImg()),"test");
...
void MainWindow::addContourToViwer(QPolygonF *polycontour)
{
    if( _GetStatusImg() < contoursList.count())
    {

        contoursList[_GetStatusImg()] = *polycontour;
        qDebug()<<__FUNCTION__<<_GetStatusImg()<<contoursList.at(_GetStatusImg());
    }
