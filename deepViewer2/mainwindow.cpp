...
{
    QString boxstr =  QString("%1, <%2,%3,%4,%5>")
            .arg(map_boxclas[QBoxitem::BoxClass(box->id_class)])
            .arg((int)(box->left),4)
            .arg((int)(box->top),4)
...
//    BOX_CLASS_SIZE
    ...
    else if( m_nClassId == QBoxitem::LOWERCASE )
    {
...
void MainWindow::triggeredJawBone()
{
    scene->changeBoxClass(QBoxitem::BoxClass::JAWBONE);
    m_nClassId = QBoxitem::JAWBONE;
}

void MainWindow::triggeredBackground()
{
    scene->changeBoxClass(QBoxitem::BoxClass::BACKGROUND);
    m_nClassId = QBoxitem::BACKGROUND;
}

void MainWindow::triggeredEndLowercase()
{
    scene->changeBoxClass(QBoxitem::BoxClass::END_LOWERCASE);
    m_nClassId = QBoxitem::END_LOWERCASE;
}


void MainWindow::triggeredMentalNerve()
{
    scene->changeBoxClass(QBoxitem::BoxClass::MENTALNERVE);
    m_nClassId = QBoxitem::MENTALNERVE;
}
...
void MainWindow::createToolBarActions()
{
    ...
    ActionNerve   =   new QAction(map_boxclas[QBoxitem::NERVE],this);
    ActionNerve->setData(QBoxitem::NERVE);

    qDebug()<<__FUNCTION__<<"------------>"<<sizeof(QBoxitem::BoxClass);
    ActionLowerCase  =   new QAction(map_boxclas[QBoxitem::LOWERCASE],this);
    ActionLowerCase->setData(QBoxitem::LOWERCASE);

    ActionMentalNerve   =   new QAction(map_boxclas[QBoxitem::MENTALNERVE], this);
    ActionMentalNerve->setData(QBoxitem::MENTALNERVE);

    ActionBackground   =   new QAction(map_boxclas[QBoxitem::BACKGROUND], this);
    ActionBackground->setData(QBoxitem::BACKGROUND);

    ActionJawBone   =   new QAction(map_boxclas[QBoxitem::JAWBONE], this);
    ActionJawBone->setData(QBoxitem::JAWBONE);

    ActionEndLowercase = new QAction(map_boxclas[QBoxitem::END_LOWERCASE], this);
    ActionEndLowercase->setData(QBoxitem::END_LOWERCASE);

    ActionNerve->setIcon(QIcon(":/icons/nerve.png"));
    ActionLowerCase->setIcon(QIcon(":/icons/toothbone.png"));    
    ActionMentalNerve->setIcon(QIcon(":/icons/mentalnerve.png"));
    ActionBackground->setIcon(QIcon(":/icons/background.png"));
    ActionJawBone->setIcon(QIcon(":/icons/jawbone.png"));
    ActionEndLowercase->setIcon(QIcon(":/icons/end.png"));
    ...
    connect(ActionJawBone  , SIGNAL(triggered()), this, SLOT(triggeredJawBone()));
    connect(ActionMentalNerve  , SIGNAL(triggered()), this, SLOT(triggeredMentalNerve()));
    connect(ActionBackground  , SIGNAL(triggered()), this, SLOT(triggeredBackground()));
    connect(ActionEndLowercase  , SIGNAL(triggered()), this, SLOT(triggeredEndLowercase()));
...
void MainWindow::createToolBarMenu()
{
    ...
    ClassViewMenu->addAction(ActionJawBone);
    ClassViewMenu->addAction(ActionMentalNerve);    
    ClassViewMenu->addAction(ActionEndLowercase);
    ClassViewMenu->addAction(ActionBackground);
