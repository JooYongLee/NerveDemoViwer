...
//   ;
//    std::map<QBoxitem::BoxClass,QString>::iterator item = map_boxclas.begin();
//    for ( item ; item != map_boxclas.end(); item++)
//    {
//    }
    ...
    for(int k = 0; k < boxfilelist.size(); k ++)
    {
        ...
        QJsonObject item;
        QJsonArray boxes[QBoxitem::BOX_CLASS_SIZE];
        int count[QBoxitem::BOX_CLASS_SIZE] = {0,};
        ...
        int totalCount = 0;
        for( int box_id = 0; box_id < QBoxitem::BOX_CLASS_SIZE; box_id++)
        {
            qDebug()<<__FUNCTION__<<k<<count[box_id];
            if( count[box_id] > 0 )
            {
                QString str = map_boxclas[QBoxitem::BoxClass(box_id)];
                item[str] = boxes[box_id];
                totalCount += count[box_id];
//                item[map_boxclas[box_id]] = boxes[box_id];
            }
...
//        qDebug()<<__FUNCTION__<<"json array";
        ...
        foreach (QString key, keylist) {
            ...
            for ( item ; item != map_boxclas.end(); item++)
            {
                if(!lowerkey.compare(item->second))
                {
                    boxarray = boxes_a_filse[key].toArray();
                    break;
//                    boxitem.setClass(QBoxitem::BoxClass(item->first));
                }
            }
